          ...
          for (const strategyId in userOrders) {
            ...
            for (const order of strategyOrders) {
              ...
              try {
                ...
                const exchangeCode = (strategyParams?.exchangeCode as 'NASD' | 'NYSE' | 'AMEX') || 'NASD';
                ...
                if (newStatus !== order.status) {
                  ...
                  if (strategy.type === 'LOO_LOC' && newStatus === 'FILLED' && order.side === 'BUY') {
                    // This is a simplified recalculation. A more robust solution would fetch all filled orders for the strategy.
                    // For now, we'll just update the strategy's parameters with the new average cost.
