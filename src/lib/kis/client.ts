...
export class KISClient {
  ...
  async submitOrder(params: OrderParams): Promise<OrderResponse> {
    ...
    if (params.market === 'KR') {
      ...
      const isLooLocOrder = params.orderType === 'LOO' || params.orderType === 'LOC';

      path = '/uapi/overseas-stock/v1/trading/order';

      // TR_ID 선택 (모의투자/실거래, 매수/매도)
      // LOO/LOC도 일반 주문과 동일한 TR_ID 사용 (ORD_DVSN으로 구분)
      if (params.side === 'BUY') {
      ...
      // 공식 문서: 00=지정가, 32=LOO(장개시지정가), 34=LOC(장마감지정가)
      // 매도 전용: 31=MOO(장개시시장가), 33=MOC(장마감시장가)
      let orderDivisionCode = '00'; // 기본값: 지정가

      if (params.orderType === 'MARKET') {
        orderDivisionCode = '01'; // 시장가
      } else if (isLooLocOrder && !this.config.isMock) {
        // 실거래에서 LOO/LOC 주문
        orderDivisionCode = params.orderType === 'LOO' ? '32' : '34';
      } else if (isLooLocOrder && this.config.isMock) {
        // 모의투자에서는 LOO, LOC를 지정가('00')로 처리 (모의투자 미지원)
      ...
      bodyBase = {
        CANO: accNo,
        ACNT_PRDT_CD: accCode,
        OVRS_EXCG_CD: params.exchangeCode || 'NASD',
        PDNO: symbolUpper,
        ORD_DVSN: orderDivisionCode,
        ORD_QTY: params.quantity.toString(),
