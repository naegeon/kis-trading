...
import { executionLogs, logLevelEnum } from '@/lib/db/schema';
...
export async function log(
  ...
  // INFO messages are only stored if they are cron-related or have a strategyId
  const shouldStoreInDb =
    level === 'WARN' ||
    level === 'ERROR' ||
    (level === 'INFO' && (message.includes('Cron job') || strategyId));
